...
public class XSConstraints {
    ...
    /* renamed from: mf.org.apache.xerces.impl.xs.XSConstraints.1 */
    class C08831 implements Comparator {
        C08831() {
        }
    ...
    static {
        STRING_TYPE = (XSSimpleType) SchemaGrammar.SG_SchemaNS.getGlobalTypeDecl(SchemaSymbols.ATTVAL_STRING);
        fEmptyParticle = null;
        ELEMENT_PARTICLE_COMPARATOR = new C08831();
    }
    ...
    public static void fullSchemaChecking(XSGrammarBucket grammarBucket, SubstitutionGroupHandler SGHandler, CMBuilder cmBuilder, XMLErrorReporter errorReporter) {
        ...
        for (int g = grammars.length - 1; g >= 0; g--) {
            ...
            while (i < redefinedGroups.length) {
                ...
                if (baseMG == null) {
                    if (derivedMG != null) {
                        simpleLocator = rgLocators[(i / 2) - 1];
                        strArr = new Object[2];
                        strArr[0] = derivedGrp.fName;
                        strArr[1] = "rcase-Recurse.2";
                        reportSchemaError(errorReporter, simpleLocator, "src-redefine.6.2.2", strArr);
                    }
        ...
        for (i = grammars.length - 1; i >= 0; i--) {
            ...
            while (j < types.length) {
                ...
                if (!(types[j].fBaseType == null || types[j].fBaseType == SchemaGrammar.fAnyType)) {
                    short s = types[j].fDerivedBy;
                    if (r0 == (short) 2) {
                        if (types[j].fBaseType instanceof XSComplexTypeDecl) {
                            ...
                            if (derivedParticle == null) {
                                if (!(baseParticle == null || baseParticle.emptiable())) {
                                    simpleLocator = ctLocators[j];
                                    strArr = new Object[2];
                                    strArr[0] = types[j].fName;
                                    strArr[1] = types[j].fBaseType.getName();
                                    reportSchemaError(errorReporter, simpleLocator, "derivation-ok-restriction.5.3.2", strArr);
                                }
                                ...
                                try {
                                    ...
                                    simpleLocator = ctLocators[j];
                                    objArr = new Object[1];
                                    objArr[0] = types[j].fName;
                                    reportSchemaError(errorReporter, simpleLocator, "derivation-ok-restriction.5.4.2", objArr);
                                }
                            } else {
                                simpleLocator = ctLocators[j];
                                objArr = new Object[1];
    ...
    private static boolean particleValidRestriction(XSParticleDecl dParticle, SubstitutionGroupHandler dSGHandler, XSParticleDecl bParticle, SubstitutionGroupHandler bSGHandler, boolean checkWCOccurrence) throws XMLSchemaException {
        ...
        if (dParticle.isEmpty() && !bParticle.emptiable()) {
                        ...
                        switch (dType) {
                            ...
                            case HttpStatus.SC_SWITCHING_PROTOCOLS /*101*/:
                                switch (bType) {
                                    ...
                                    case DurationDV.DAYTIMEDURATION_TYPE /*2*/:
                                        if (dMinEffectiveTotalRange2 != OCCURRENCE_UNKNOWN) {
                                            dMinEffectiveTotalRange = dParticle.minEffectiveTotalRange();
                                        } else {
                                            dMinEffectiveTotalRange = dMinEffectiveTotalRange2;
                                        }
                                        if (dMaxEffectiveTotalRange2 != OCCURRENCE_UNKNOWN) {
            ...
            dMaxEffectiveTotalRange2 = OCCURRENCE_UNKNOWN;
            dMinEffectiveTotalRange2 = OCCURRENCE_UNKNOWN;
            dChildren2 = dChildren;
            bType = bParticle.fType;
            if (bType == (short) 3) {
                bType = ((XSModelGroupImpl) bParticle.fValue).fCompositor;
