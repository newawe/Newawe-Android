...
public class XSDFACM implements XSCMValidator {
    ...
    private void buildDFA(CMNode syntaxTree) {
        ...
        this.fLeafCount = i + 1;
        this.fHeadNode = new XSCMBinOp(HttpStatus.SC_PROCESSING, syntaxTree, new XSCMLeaf(1, null, -1, i));
        ...
        if (elemOccurenceMap != null) {
            ...
            for (int i3 = 0; i3 < curState; i3++) {
                int[] transitions = this.fTransTable[i3];
                for (int j = 0; j < transitions.length; j++) {
                    if (i3 == transitions[j]) {
                        this.fCountingStates[i3] = elemOccurenceMap[j];
                        break;
                    }
                }
            }
    ...
    private void calcFollowList(CMNode nodeCur) {
        if (nodeCur.type() == HttpStatus.SC_SWITCHING_PROTOCOLS) {
            calcFollowList(((XSCMBinOp) nodeCur).getLeft());
            calcFollowList(((XSCMBinOp) nodeCur).getRight());
        } else if (nodeCur.type() == HttpStatus.SC_PROCESSING) {
            calcFollowList(((XSCMBinOp) nodeCur).getLeft());
            calcFollowList(((XSCMBinOp) nodeCur).getRight());
    ...
    private void postTreeBuildInit(CMNode nodeCur) throws RuntimeException {
        ...
        } else if (nodeCur.type() == HttpStatus.SC_SWITCHING_PROTOCOLS || nodeCur.type() == HttpStatus.SC_PROCESSING) {
        ...
        while (i < this.fTransTable.length && this.fTransTable[i] != null) {
            ...
            while (j < this.fElemMapSize) {
                ...
                while (k < this.fElemMapSize) {
                    if (!(this.fTransTable[i][j] == -1 || this.fTransTable[i][k] == -1 || conflictTable[j][k] != null)) {
