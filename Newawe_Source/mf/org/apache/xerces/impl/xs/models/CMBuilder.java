...
public class CMBuilder {
    ...
    public XSCMValidator getContentModel(XSComplexTypeDecl typeDecl, boolean forUPA) {
        ...
        if (particle.fType == (short) 3 && ((XSModelGroupImpl) particle.fValue).fCompositor == XSModelGroupImpl.MODELGROUP_ALL) {
    ...
    private CMNode expandContentModel(CMNode node, int minOccurs, int maxOccurs) {
        ...
        if (maxOccurs == -1) {
            return this.fNodeFactory.getCMBinOpNode(HttpStatus.SC_PROCESSING, multiNodes(node, minOccurs - 1, true), this.fNodeFactory.getCMUniOpNode(6, node));
        }
        if (minOccurs > 0) {
        ...
        return this.fNodeFactory.getCMBinOpNode(HttpStatus.SC_PROCESSING, nodeRet, multiNodes(node, maxOccurs - minOccurs, true));
    }
    ...
    private CMNode multiNodes(CMNode node, int num, boolean copyFirst) {
        ...
        if (num != 1) {
            int num1 = num / 2;
            return this.fNodeFactory.getCMBinOpNode(HttpStatus.SC_PROCESSING, multiNodes(node, num1, copyFirst), multiNodes(node, num - num1, true));
        } else if (copyFirst) {
    ...
    private CMNode buildCompactSyntaxTree(XSParticleDecl particle) {
        ...
        if (type == (short) 3) {
            ...
            if (nodeRet != null && group.fCompositor == XSModelGroupImpl.MODELGROUP_CHOICE && count < group.fParticleCount) {
                nodeRet = this.fNodeFactory.getCMUniOpNode(5, nodeRet);
            }
        }
        return nodeRet;
