...
public class SchemaDOM extends DefaultDocument {
    ...
    private void resizeRelations() {
        NodeImpl[][] temp = new NodeImpl[(this.relations.length + relationsRowResizeFactor)][];
    ...
    private void resizeRelations(int i) {
        NodeImpl[] temp = new NodeImpl[(this.relations[i].length + relationsColResizeFactor)];
    ...
    public void reset() {
        ...
        this.relations = new NodeImpl[relationsRowResizeFactor][];
        this.parent = new ElementImpl(0, 0, 0);
        this.parent.rawname = "DOCUMENT_NODE";
        this.currLoc = 0;
        this.nextFreeLoc = 1;
        this.inCDATA = false;
        for (i = 0; i < relationsRowResizeFactor; i++) {
            this.relations[i] = new NodeImpl[relationsColResizeFactor];
        }
    ...
    void startAnnotation(String elemRawName, XMLAttributes attributes, NamespaceContext namespaceContext) {
        if (this.fAnnotationBuffer == null) {
            this.fAnnotationBuffer = new StringBuffer(NodeFilter.SHOW_DOCUMENT);
        }
        this.fAnnotationBuffer.append("<").append(elemRawName).append(" ");
