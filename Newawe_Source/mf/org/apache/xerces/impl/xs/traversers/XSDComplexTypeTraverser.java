...
class XSDComplexTypeTraverser extends XSDAbstractParticleTraverser {
    ...
    XSComplexTypeDecl traverseLocal(Element complexTypeNode, XSDocumentInfo schemaDoc, SchemaGrammar grammar) {
        Object[] attrValues = this.fAttrChecker.checkAttributes(complexTypeNode, DEBUG, schemaDoc);
    ...
    private XSComplexTypeDecl traverseComplexTypeDecl(Element complexTypeDecl, String complexTypeName, Object[] attrValues, XSDocumentInfo schemaDoc, SchemaGrammar grammar) {
        ...
        boolean z2 = (abstractAtt == null || !abstractAtt.booleanValue()) ? DEBUG : true;
        this.fIsAbstract = z2;
        this.fAnnotations = null;
        try {
            Element child;
            ...
            if (child2 != null) {
                try {
                    if (DOMUtil.getLocalName(child2).equals(SchemaSymbols.ELT_ANNOTATION)) {
                        addAnnotation(traverseAnnotationDecl(child2, attrValues, DEBUG, schemaDoc));
                        child = DOMUtil.getNextSiblingElement(child2);
                    } else {
                        text = DOMUtil.getSyntheticAnnotation(complexTypeDecl);
                        if (text != null) {
                            addAnnotation(traverseSyntheticAnnotation(complexTypeDecl, text, attrValues, DEBUG, schemaDoc));
                        }
                        child = child2;
                    }
                    if (child != null) {
                    ...
                    if (this.fAnnotations == null) {
                        length = this.fAnnotations.length;
                    } else {
                        length = 0;
                    }
                    xSComplexTypeDecl.setValues(str, str2, xSTypeDefinition, s, s2, s3, s4, z, xSAttributeGroupDecl, xSSimpleType, xSParticleDecl, new XSObjectListImpl(xSObjectArr, length));
            ...
            if (child == null) {
                ...
                this.fDerivedBy = (short) 2;
                processComplexContent(child, mixedAtt.booleanValue(), DEBUG, schemaDoc, grammar);
    ...
    private void traverseSimpleContent(Element simpleContentElement, XSDocumentInfo schemaDoc, SchemaGrammar grammar) throws ComplexTypeRecoverableError {
        String text;
        Object[] simpleContentAttrValues = this.fAttrChecker.checkAttributes(simpleContentElement, DEBUG, schemaDoc);
        ...
        if (simpleContent == null || !DOMUtil.getLocalName(simpleContent).equals(SchemaSymbols.ELT_ANNOTATION)) {
            ...
            if (text != null) {
                addAnnotation(traverseSyntheticAnnotation(simpleContentElement, text, simpleContentAttrValues, DEBUG, schemaDoc));
            }
        } else {
            addAnnotation(traverseAnnotationDecl(simpleContent, simpleContentAttrValues, DEBUG, schemaDoc));
        ...
        Object[] derivationTypeAttrValues = this.fAttrChecker.checkAttributes(simpleContent, DEBUG, schemaDoc);
        ...
        if (simpleContent != null) {
            if (DOMUtil.getLocalName(simpleContent).equals(SchemaSymbols.ELT_ANNOTATION)) {
                addAnnotation(traverseAnnotationDecl(simpleContent, derivationTypeAttrValues, DEBUG, schemaDoc));
                simpleContent = DOMUtil.getNextSiblingElement(simpleContent);
            } else {
                text = DOMUtil.getSyntheticAnnotation(scElement);
                if (text != null) {
                    addAnnotation(traverseSyntheticAnnotation(scElement, text, derivationTypeAttrValues, DEBUG, schemaDoc));
                }
            }
            if (simpleContent != null && DOMUtil.getLocalName(simpleContent).equals(SchemaSymbols.ELT_ANNOTATION)) {
                this.fAttrChecker.returnAttrArray(simpleContentAttrValues, schemaDoc);
        ...
        if (this.fDerivedBy == (short) 2) {
            ...
            try {
                mergeAttributes(baseComplexType.getAttrGrp(), this.fAttrGrp, this.fName, DEBUG, simpleContentElement);
    ...
    private void traverseComplexContent(Element complexContentElement, boolean mixedOnType, XSDocumentInfo schemaDoc, SchemaGrammar grammar) throws ComplexTypeRecoverableError {
        ...
        Object[] complexContentAttrValues = this.fAttrChecker.checkAttributes(complexContentElement, DEBUG, schemaDoc);
        ...
        if (complexContent2 == null || !DOMUtil.getLocalName(complexContent2).equals(SchemaSymbols.ELT_ANNOTATION)) {
            ...
            if (text != null) {
                addAnnotation(traverseSyntheticAnnotation(complexContentElement, text, complexContentAttrValues, DEBUG, schemaDoc));
            }
            complexContent = complexContent2;
        } else {
            addAnnotation(traverseAnnotationDecl(complexContent2, complexContentAttrValues, DEBUG, schemaDoc));
        ...
        Object[] derivationTypeAttrValues = this.fAttrChecker.checkAttributes(complexContent, DEBUG, schemaDoc);
        QName baseTypeName = derivationTypeAttrValues[XSAttributeChecker.ATTIDX_BASE];
        if (baseTypeName == null) {
            this.fAttrChecker.returnAttrArray(complexContentAttrValues, schemaDoc);
            this.fAttrChecker.returnAttrArray(derivationTypeAttrValues, schemaDoc);
        ...
        if (type == null) {
            ...
            if (complexContent != null) {
                if (DOMUtil.getLocalName(complexContent).equals(SchemaSymbols.ELT_ANNOTATION)) {
                    addAnnotation(traverseAnnotationDecl(complexContent, derivationTypeAttrValues, DEBUG, schemaDoc));
                    complexContent = DOMUtil.getNextSiblingElement(complexContent);
                } else {
                    text = DOMUtil.getSyntheticAnnotation(complexContent);
                    if (text != null) {
                        addAnnotation(traverseSyntheticAnnotation(complexContent, text, derivationTypeAttrValues, DEBUG, schemaDoc));
                    }
                }
            ...
            try {
                ...
                if (this.fDerivedBy != (short) 2) {
                    if (this.fParticle == null) {
                        ...
                        } else if ((this.fParticle.fType == (short) 3 && ((XSModelGroupImpl) this.fParticle.fValue).fCompositor == XSModelGroupImpl.MODELGROUP_ALL) || (((XSParticleDecl) baseType.getParticle()).fType == (short) 3 && ((XSModelGroupImpl) ((XSParticleDecl) baseType.getParticle()).fValue).fCompositor == XSModelGroupImpl.MODELGROUP_ALL)) {
                            this.fAttrChecker.returnAttrArray(complexContentAttrValues, schemaDoc);
                            this.fAttrChecker.returnAttrArray(derivationTypeAttrValues, schemaDoc);
                    ...
                    try {
                        mergeAttributes(baseType.getAttrGrp(), this.fAttrGrp, this.fName, DEBUG, complexContent);
                        this.fAttrGrp.removeProhibitedAttrs();
    ...
    private void contentBackup() {
        ...
        if (this.fGlobalStorePos == this.fGlobalStore.length) {
            Object[] newArray = new Object[(this.fGlobalStorePos + GLOBAL_NUM)];
