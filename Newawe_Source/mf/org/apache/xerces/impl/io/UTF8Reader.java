...
public final class UTF8Reader extends Reader {
    ...
    public UTF8Reader(InputStream inputStream) {
        this(inputStream, (int) DEFAULT_BUFFER_SIZE, new XMLMessageFormatter(), Locale.getDefault());
    }
    ...
    public int read() throws IOException {
        ...
        if (this.fSurrogate == -1) {
            ...
            if (0 == this.fOffset) {
                ...
                index = 0 + 1;
                b0 = this.fBuffer[0] & MotionEventCompat.ACTION_MASK;
            }
            if (b0 == -1) {
                return -1;
            }
            if (b0 < TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                ...
                if (index == this.fOffset) {
                    ...
                    r6 = index + 1;
                    b1 = this.fBuffer[index] & MotionEventCompat.ACTION_MASK;
                }
                if (b1 == -1) {
                    expectedByte(2, 2);
                }
                if ((b1 & 192) != TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                    invalidByte(2, 2, b1);
                }
                c = ((b0 << 6) & 1984) | (b1 & 63);
            } else if ((b0 & 240) == 224) {
                ...
                if ((b1 & 192) != TransportMediator.FLAG_KEY_MEDIA_NEXT || ((b0 == 237 && b1 >= 160) || ((b0 & 15) == 0 && (b1 & 32) == 0))) {
                    invalidByte(2, 3, b1);
                }
                if (index == this.fOffset) {
                    ...
                    r6 = index + 1;
                    b2 = this.fBuffer[index] & MotionEventCompat.ACTION_MASK;
                }
                if (b2 == -1) {
                    expectedByte(3, 3);
                }
                ...
                if ((b1 & 192) != TransportMediator.FLAG_KEY_MEDIA_NEXT || ((b1 & 48) == 0 && (b0 & 7) == 0)) {
                    invalidByte(2, 3, b1);
                }
                if (index == this.fOffset) {
                    b2 = this.fInputStream.read();
                ...
                if (index == this.fOffset) {
                    ...
                    r6 = index + 1;
                    b3 = this.fBuffer[index] & MotionEventCompat.ACTION_MASK;
                }
                if (b3 == -1) {
                    expectedByte(4, 4);
                }
                if ((b3 & 192) != TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                    invalidByte(4, 4, b3);
                }
    ...
    public int read(char[] ch, int offset, int length) throws IOException {
        ...
        while (in < total) {
            ...
            if (byte1 >= null) {
                ...
                int b0 = byte1 & MotionEventCompat.ACTION_MASK;
                i = b0 & 224;
                int b1;
                if (r0 != 192 || (b0 & 30) == 0) {
                    i = b0 & 240;
                    ...
                    if (r0 == 224) {
                        in++;
                        if (in < total) {
                            b1 = this.fBuffer[in] & MotionEventCompat.ACTION_MASK;
                        } else {
                            b1 = this.fInputStream.read();
                            if (b1 == -1) {
                                if (out > offset) {
                        ...
                        i = b1 & 192;
                        if (r0 != 128 || ((b0 == 237 && b1 >= 160) || ((b0 & 15) == 0 && (b1 & 32) == 0))) {
                            if (out > offset) {
                                this.fBuffer[0] = (byte) b0;
                        ...
                        in++;
                        if (in < total) {
                            b2 = this.fBuffer[in] & MotionEventCompat.ACTION_MASK;
                        } else {
                            b2 = this.fInputStream.read();
                            if (b2 == -1) {
                                if (out > offset) {
                        ...
                        if (r0 == 240) {
                            ...
                            in++;
                            if (in < total) {
                                b3 = this.fBuffer[in] & MotionEventCompat.ACTION_MASK;
                            } else {
                                b3 = this.fInputStream.read();
                                if (b3 == -1) {
                                    if (out > offset) {
    ...
    public void mark(int readAheadLimit) throws IOException {
        throw new IOException(this.fFormatter.formatMessage(this.fLocale, "OperationNotSupported", new Object[]{"mark()", HTTP.UTF_8}));
    }
    ...
    private void expectedByte(int position, int count) throws MalformedByteSequenceException {
        throw new MalformedByteSequenceException(this.fFormatter, this.fLocale, XMLMessageFormatter.XML_DOMAIN, "ExpectedByte", new Object[]{Integer.toString(position), Integer.toString(count)});
    }

    private void invalidByte(int position, int count, int c) throws MalformedByteSequenceException {
        throw new MalformedByteSequenceException(this.fFormatter, this.fLocale, XMLMessageFormatter.XML_DOMAIN, "InvalidByte", new Object[]{Integer.toString(position), Integer.toString(count)});
    }

    private void invalidSurrogate(int uuuuu) throws MalformedByteSequenceException {
        throw new MalformedByteSequenceException(this.fFormatter, this.fLocale, XMLMessageFormatter.XML_DOMAIN, "InvalidHighSurrogate", new Object[]{Integer.toHexString(uuuuu)});
    }
}
