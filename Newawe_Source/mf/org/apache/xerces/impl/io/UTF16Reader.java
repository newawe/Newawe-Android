...
public final class UTF16Reader extends Reader {
    ...
    public UTF16Reader(InputStream inputStream, boolean isBigEndian) {
        this(inputStream, (int) DEFAULT_BUFFER_SIZE, isBigEndian, new XMLMessageFormatter(), Locale.getDefault());
    }

    public UTF16Reader(InputStream inputStream, boolean isBigEndian, MessageFormatter messageFormatter, Locale locale) {
        this(inputStream, (int) DEFAULT_BUFFER_SIZE, isBigEndian, messageFormatter, locale);
    }
    ...
    public void mark(int readAheadLimit) throws IOException {
        throw new IOException(this.fFormatter.formatMessage(this.fLocale, "OperationNotSupported", new Object[]{"mark()", HTTP.UTF_16}));
    }
    ...
    private void processBE(char[] ch, int offset, int count) {
        ...
        while (i < count) {
            ...
            curPos = curPos2 + 1;
            offset = offset2 + 1;
            ch[offset2] = (char) ((b0 << 8) | (this.fBuffer[curPos2] & MotionEventCompat.ACTION_MASK));
            i++;
            offset2 = offset;
        }
    }
    ...
    private void processLE(char[] ch, int offset, int count) {
        ...
        while (i < count) {
            ...
            curPos = curPos2 + 1;
            offset = offset2 + 1;
            ch[offset2] = (char) (((this.fBuffer[curPos2] & MotionEventCompat.ACTION_MASK) << 8) | b0);
            i++;
            offset2 = offset;
        }
    }
    ...
    private void expectedTwoBytes() throws MalformedByteSequenceException {
        throw new MalformedByteSequenceException(this.fFormatter, this.fLocale, XMLMessageFormatter.XML_DOMAIN, "ExpectedByte", new Object[]{"2", "2"});
    }
}
