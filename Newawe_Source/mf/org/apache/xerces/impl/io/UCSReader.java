...
public final class UCSReader extends Reader {
    ...
    public UCSReader(InputStream inputStream, short encoding) {
        this(inputStream, (int) DEFAULT_BUFFER_SIZE, encoding);
    }
    ...
    public int read() throws IOException {
        int b0 = this.fInputStream.read() & MotionEventCompat.ACTION_MASK;
        if (b0 == MotionEventCompat.ACTION_MASK) {
            return -1;
        }
        int b1 = this.fInputStream.read() & MotionEventCompat.ACTION_MASK;
        if (b1 == MotionEventCompat.ACTION_MASK) {
            return -1;
        }
        if (this.fEncoding >= UCS4LE) {
            int b2 = this.fInputStream.read() & MotionEventCompat.ACTION_MASK;
            if (b2 == MotionEventCompat.ACTION_MASK) {
                return -1;
            }
            int b3 = this.fInputStream.read() & MotionEventCompat.ACTION_MASK;
            if (b3 == MotionEventCompat.ACTION_MASK) {
                return -1;
            }
    ...
    public int read(char[] ch, int offset, int length) throws IOException {
        ...
        while (i < numChars) {
            ...
            int b0 = this.fBuffer[curPos] & MotionEventCompat.ACTION_MASK;
            curPos = curPos2 + 1;
            int b1 = this.fBuffer[curPos2] & MotionEventCompat.ACTION_MASK;
            if (this.fEncoding >= UCS4LE) {
                curPos2 = curPos + 1;
                int b2 = this.fBuffer[curPos] & MotionEventCompat.ACTION_MASK;
                curPos = curPos2 + 1;
                int b3 = this.fBuffer[curPos2] & MotionEventCompat.ACTION_MASK;
                if (this.fEncoding == UCS4BE) {
