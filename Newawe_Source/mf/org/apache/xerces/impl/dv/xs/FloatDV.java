            ...
            if (this.canonical == null) {
                if (this.value == Float.POSITIVE_INFINITY) {
                    ...
                    if (this.canonical.indexOf(69) == -1) {
                        ...
                        int i;
                        int i2;
                        if (this.value >= DefaultRetryPolicy.DEFAULT_BACKOFF_MULT || this.value <= -1.0f) {
                            int dp = this.canonical.indexOf(46);
                            for (i = dp; i > edp; i--) {
                                chars[i] = chars[i - 1];
                            }
                            chars[edp] = ClassUtils.PACKAGE_SEPARATOR_CHAR;
                            while (chars[len - 1] == '0') {
                                len--;
                            }
                            if (chars[len - 1] == ClassUtils.PACKAGE_SEPARATOR_CHAR) {
                                i2 = len + 1;
                            } else {
                                i2 = len;
                            }
