...
public final class REUtil {
    ...
    static final int composeFromSurrogates(int high, int low) {
        return ((AccessibilityNodeInfoCompat.ACTION_CUT + ((high - 55296) << 10)) + low) - 56320;
    }
    ...
    static final String decomposeToSurrogates(int ch) {
        chs = new char[2];
        ch -= AccessibilityNodeInfoCompat.ACTION_CUT;
    ...
    static final int getOptionValue(int ch) {
        switch (ch) {
            ...
            case C0302R.styleable.Theme_colorButtonNormal /*88*/:
                return XMLEntityManager.DEFAULT_INTERNAL_BUFFER_SIZE;
    ...
    static final String createOptionString(int options) {
        ...
        if ((options & NodeFilter.SHOW_DOCUMENT) != 0) {
            sb.append('F');
        }
        if ((options & TransportMediator.FLAG_KEY_MEDIA_NEXT) != 0) {
            sb.append('H');
        }
        if ((options & XMLEntityManager.DEFAULT_INTERNAL_BUFFER_SIZE) != 0) {
            sb.append('X');
        }
        if ((options & 2) != 0) {
            sb.append('i');
        ...
        if ((options & NodeFilter.SHOW_DOCUMENT_FRAGMENT) != 0) {
            sb.append(',');
        }
        ...
        try {
            ...
            while (i < argv.length) {
                if (argv[i].length() == 0 || argv[i].charAt(0) != '-') {
                    ...
                    options = new StringBuilder(String.valueOf(options)).append("i").toString();
                } else if (argv[i].equals("-m")) {
                    options = new StringBuilder(String.valueOf(options)).append("m").toString();
                } else if (argv[i].equals("-s")) {
                    options = new StringBuilder(String.valueOf(options)).append("s").toString();
                } else if (argv[i].equals("-u")) {
                    options = new StringBuilder(String.valueOf(options)).append("u").toString();
    ...
    static {
        regexCache = new RegularExpression[CACHESIZE];
    }
