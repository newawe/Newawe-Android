...
public abstract class CharacterDataImpl extends ChildNode {
    ...
    /* renamed from: mf.org.apache.xerces.dom.CharacterDataImpl.1 */
    class C12621 implements NodeList {
        C12621() {
        }
    ...
    void internalDeleteData(int offset, int count, boolean replace) throws DOMException {
        ...
        try {
            setNodeValueInternal(new StringBuilder(String.valueOf(this.data.substring(0, offset))).append(tailLength > 0 ? this.data.substring(offset + count, (offset + count) + tailLength) : StringUtils.EMPTY).toString(), replace);
