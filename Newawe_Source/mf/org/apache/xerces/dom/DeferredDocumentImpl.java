...
public class DeferredDocumentImpl extends DocumentImpl implements DeferredNode {
    ...
    public String getDeferredEntityBaseURI(int entityIndex) {
        if (entityIndex != -1) {
            return getNodeName(getNodeExtra(getNodeExtra(entityIndex, DEBUG_PRINT_TABLES), DEBUG_PRINT_TABLES), DEBUG_PRINT_TABLES);
        }
        return null;
    }

    public void setEntityInfo(int currentEntityDecl, String version, String encoding) {
        int eNodeIndex = getNodeExtra(currentEntityDecl, DEBUG_PRINT_TABLES);
        if (eNodeIndex != -1) {
    ...
    public void setTypeInfo(int elementNodeIndex, Object type) {
        setChunkValue(this.fNodeValue, type, elementNodeIndex >> CHUNK_SHIFT, elementNodeIndex & CHUNK_MASK);
    }

    public void setInputEncoding(int currentEntityDecl, String value) {
        int extraDataIndex = getNodeExtra(getNodeExtra(currentEntityDecl, DEBUG_PRINT_TABLES), DEBUG_PRINT_TABLES);
        setChunkValue(this.fNodeValue, value, extraDataIndex >> CHUNK_SHIFT, extraDataIndex & CHUNK_MASK);
    }
    ...
    public int setDeferredAttribute(int elementNodeIndex, String attrName, String attrURI, String attrValue, boolean specified, boolean id, Object type) {
        ...
        if (id) {
            extra |= XMLEntityManager.DEFAULT_INTERNAL_BUFFER_SIZE;
    ...
    public int createDeferredAttribute(String attrName, String attrURI, String attrValue, boolean specified) {
        ...
        setChunkIndex(this.fNodeExtra, specified ? INITIAL_CHUNK_COUNT : 0, chunk, index);
        return nodeIndex;
    }

    public int createDeferredElementDefinition(String elementName) {
        int nodeIndex = createNode((short) 21);
        setChunkValue(this.fNodeName, elementName, nodeIndex >> CHUNK_SHIFT, nodeIndex & CHUNK_MASK);
        return nodeIndex;
    }
    ...
    public int createDeferredCDATASection(String data) {
        ...
        setChunkValue(this.fNodeValue, data, nodeIndex >> CHUNK_SHIFT, nodeIndex & CHUNK_MASK);
        return nodeIndex;
    }
    ...
    public int cloneNode(int nodeIndex, boolean deep) {
        ...
        if (extraIndex != -1) {
            if (!(nodeType == 2 || nodeType == 3)) {
                extraIndex = cloneNode(extraIndex, DEBUG_PRINT_TABLES);
            }
            setChunkIndex(this.fNodeExtra, extraIndex, cchunk, cindex);
        }
        if (deep) {
            int prevIndex = -1;
            int childIndex = getLastChild(nodeIndex, DEBUG_PRINT_TABLES);
            while (childIndex != -1) {
                ...
                childIndex = getRealPrevSibling(childIndex, DEBUG_PRINT_TABLES);
            }
        }
        return cloneIndex;
    }
    ...
    public void setIdAttributeNode(int elemIndex, int attrIndex) {
        ...
        setChunkIndex(this.fNodeExtra, getChunkIndex(this.fNodeExtra, chunk, index) | XMLEntityManager.DEFAULT_INTERNAL_BUFFER_SIZE, chunk, index);
    ...
    public void setAsLastChild(int parentIndex, int childIndex) {
        setChunkIndex(this.fNodeLastChild, childIndex, parentIndex >> CHUNK_SHIFT, parentIndex & CHUNK_MASK);
    }

    public int getParentNode(int nodeIndex) {
        return getParentNode(nodeIndex, DEBUG_PRINT_TABLES);
    }
    ...
    public int lookupElementDefinition(String elementName) {
        if (this.fNodeCount > 1) {
            ...
            index = getChunkIndex(this.fNodeLastChild, docTypeIndex >> CHUNK_SHIFT, docTypeIndex & CHUNK_MASK);
            while (index != -1) {
                nchunk = index >> CHUNK_SHIFT;
                nindex = index & CHUNK_MASK;
    ...
    public String getNodeValueString(int nodeIndex, boolean free) {
        ...
        if (type == 3) {
            ...
            if (prevSib == -1 || getNodeType(prevSib, DEBUG_PRINT_TABLES) != (short) 3) {
                return value;
            }
            ...
            } while (getNodeType(prevSib, DEBUG_PRINT_TABLES) == (short) 3);
            ...
            int child = getLastChild(nodeIndex, DEBUG_PRINT_TABLES);
            if (child == -1) {
                return value;
            }
    ...
    public Object getTypeInfo(int nodeIndex) {
        ...
        RefCount c = this.fNodeValue[chunk][CHUNK_SIZE];
        c.fCount--;
        if (c.fCount != 0) {
            return value;
        }
    ...
    public String getAttribute(int elemIndex, String name) {
        ...
        int attrIndex = getChunkIndex(this.fNodeExtra, elemIndex >> CHUNK_SHIFT, elemIndex & CHUNK_MASK);
        while (attrIndex != -1) {
            int achunk = attrIndex >> CHUNK_SHIFT;
            int aindex = attrIndex & CHUNK_MASK;
    ...
    protected void synchronizeData() {
        needsSyncData(DEBUG_PRINT_TABLES);
        if (this.fIdElement != null) {
            ...
            while (i < this.fIdCount) {
                ...
                if (idName != null) {
                    ...
                    do {
                        ...
                        index = getChunkIndex(this.fNodeParent, index >> CHUNK_SHIFT, index & CHUNK_MASK);
                    } while (index != -1);
                    Node place = this;
    ...
    protected void synchronizeChildren() {
        ...
        this.mutationEvents = DEBUG_PRINT_TABLES;
        needsSyncChildren(DEBUG_PRINT_TABLES);
        getNodeType(0);
    ...
    protected final void synchronizeChildren(AttrImpl a, int nodeIndex) {
        ...
        setMutationEvents(DEBUG_PRINT_TABLES);
        a.needsSyncChildren(DEBUG_PRINT_TABLES);
        ...
        if (getPrevSibling(last) == -1) {
            ...
            a.hasStringValue(DEBUG_PRINT_TABLES);
        }
    ...
    protected final void synchronizeChildren(ParentNode p, int nodeIndex) {
        boolean orig = getMutationEvents();
        setMutationEvents(DEBUG_PRINT_TABLES);
        p.needsSyncChildren(DEBUG_PRINT_TABLES);
        ChildNode firstNode = null;
        ChildNode lastNode = null;
    ...
    protected void ensureCapacity(int chunk) {
        if (this.fNodeType == null) {
            this.fNodeType = new int[INITIAL_CHUNK_COUNT][];
            this.fNodeName = new Object[INITIAL_CHUNK_COUNT][];
            this.fNodeValue = new Object[INITIAL_CHUNK_COUNT][];
            ...
            this.fNodeURI = new Object[INITIAL_CHUNK_COUNT][];
            this.fNodeExtra = new int[INITIAL_CHUNK_COUNT][];
    ...
    protected int createNode(short nodeType) {
        int chunk = this.fNodeCount >> CHUNK_SHIFT;
    ...
    private final int setChunkIndex(int[][] data, int value, int chunk, int index) {
        ...
        if (ovalue == -1) {
            dataChunk[CHUNK_SIZE] = dataChunk[CHUNK_SIZE] + 1;
        }
    ...
    private final String setChunkValue(Object[][] data, Object value, int chunk, int index) {
        ...
        if (ovalue == null) {
            RefCount c = dataChunk[CHUNK_SIZE];
            c.fCount++;
        }
        dataChunk[index] = value;
        return ovalue;
    ...
    private final int clearChunkIndex(int[][] data, int chunk, int index) {
        ...
        if (value != -1) {
            int[] iArr = data[chunk];
            iArr[CHUNK_SIZE] = iArr[CHUNK_SIZE] - 1;
            data[chunk][index] = -1;
    ...
    private final String clearChunkValue(Object[][] data, int chunk, int index) {
        ...
        if (value != null) {
            ...
            RefCount c = data[chunk][CHUNK_SIZE];
            c.fCount--;
            if (c.fCount == 0) {
                data[chunk] = null;
            }
