...
public final class XPointerHandler extends XIncludeHandler implements XPointerProcessor {
        ...
        private int scanNCName(String data, int endOffset, int currentOffset) {
            ...
            while (true) {
                ...
                if (currentOffset < endOffset) {
                    ...
                    if (ch < TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                        chartype = this.fASCIICharMap[ch];
                        if (!(chartype == CHARTYPE_LETTER || chartype == 9 || chartype == 7 || chartype == 6 || chartype == CHARTYPE_UNDERSCORE)) {
                            break;
                        }
    ...
    private final class Tokens {
        ...
        private Tokens(SymbolTable symbolTable) {
            ...
            this.fTokenCount = XPTRTOKEN_OPEN_PAREN;
            this.fTokenNames = new HashMap();
            this.fSymbolTable = symbolTable;
            this.fTokenNames.put(new Integer(XPTRTOKEN_OPEN_PAREN), "XPTRTOKEN_OPEN_PAREN");
            this.fTokenNames.put(new Integer(XPTRTOKEN_CLOSE_PAREN), "XPTRTOKEN_CLOSE_PAREN");
            this.fTokenNames.put(new Integer(XPTRTOKEN_SHORTHAND), "XPTRTOKEN_SHORTHAND");
            this.fTokenNames.put(new Integer(XPTRTOKEN_SCHEMENAME), "XPTRTOKEN_SCHEMENAME");
            this.fTokenNames.put(new Integer(XPTRTOKEN_SCHEMEDATA), "XPTRTOKEN_SCHEMEDATA");
        }
        ...
        private void addToken(int token) {
            try {
                ...
                this.fTokens = new int[(this.fTokenCount << XPTRTOKEN_CLOSE_PAREN)];
                System.arraycopy(oldList, XPTRTOKEN_OPEN_PAREN, this.fTokens, XPTRTOKEN_OPEN_PAREN, this.fTokenCount);
                this.fTokens[this.fTokenCount] = token;
            }
            this.fTokenCount += XPTRTOKEN_CLOSE_PAREN;
        }

        private void rewind() {
            this.fCurrentTokenIndex = XPTRTOKEN_OPEN_PAREN;
        }
        ...
        private int nextToken() throws XNIException {
            ...
            this.fCurrentTokenIndex = i + XPTRTOKEN_CLOSE_PAREN;
            return iArr[i];
        }
    ...
    /* renamed from: mf.org.apache.xerces.xpointer.XPointerHandler.1 */
    class C12731 extends Scanner {
        C12731(SymbolTable $anonymous0) {
            super($anonymous0, null);
        }
    ...
    public void parseXPointer(String xpointer) throws XNIException {
        ...
        if (!new C12731(this.fSymbolTable).scanExpr(this.fSymbolTable, tokens, xpointer, 0, xpointer.length())) {
        ...
        while (tokens.hasMore()) {
            switch (tokens.nextToken()) {
                ...
                case ConnectionResult.SERVICE_DISABLED /*3*/:
                    ...
                    while (tokens.hasMore()) {
                        if (tokens.getTokenString(tokens.nextToken()) != "XPTRTOKEN_OPEN_PAREN") {
                            schemeData = tokens.getTokenString(tokens.nextToken());
                            ...
                            elementSchemePointer = new ElementSchemePointer(this.fSymbolTable, this.fErrorReporter);
    ...
    public void setProperty(String propertyId, Object value) throws XMLConfigurationException {
        if (propertyId == XMLSchemaValidator.ERROR_REPORTER) {
            if (value != null) {
