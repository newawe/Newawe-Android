...
public class ActionBarOverlayLayout extends ViewGroup implements DecorContentParent, NestedScrollingParent {
    ...
    class C12981 extends ViewPropertyAnimatorListenerAdapter {
        C12981() {
        }
    ...
    static {
        ATTRS = new int[]{C0131R.attr.actionBarSize, 16842841};
    }
    ...
    public ActionBarOverlayLayout(Context context, AttributeSet attrs) {
        ...
        this.mTopAnimatorListener = new C12981();
    ...
    public void onWindowSystemUiVisibilityChanged(int visible) {
        ...
        if ((visible & NodeFilter.SHOW_DOCUMENT) != 0) {
            stable = true;
        } else {
            stable = false;
        }
        ...
        if ((diff & NodeFilter.SHOW_DOCUMENT) != 0 && this.mActionBarVisibilityCallback != null) {
            ViewCompat.requestApplyInsets(this);
        }
    }
    ...
    protected boolean fitSystemWindows(Rect insets) {
        pullChildren();
        boolean stable;
        if ((ViewCompat.getWindowSystemUiVisibility(this) & NodeFilter.SHOW_DOCUMENT) != 0) {
            stable = true;
        } else {
            stable = false;
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        boolean stable = (ViewCompat.getWindowSystemUiVisibility(this) & NodeFilter.SHOW_DOCUMENT) != 0;
        if (stable) {
    ...
    void pullChildren() {
        if (this.mContent == null) {
            this.mContent = (ContentFrameLayout) findViewById(C0131R.id.action_bar_activity_content);
            this.mActionBarTop = (ActionBarContainer) findViewById(C0131R.id.action_bar_container);
            this.mDecorToolbar = getDecorToolbar(findViewById(C0131R.id.action_bar));
        }
    }
    ...
    private boolean shouldHideActionBarOnFling(float velocityX, float velocityY) {
        this.mFlingEstimator.fling(0, 0, 0, (int) velocityY, 0, 0, ExploreByTouchHelper.INVALID_ID, ASContentModel.AS_UNBOUNDED);
