...
public class URLEncodedUtils {
    ...
    public static List<NameValuePair> parse(HttpEntity entity) throws IOException {
        ...
        try {
            CharArrayBuffer buf = new CharArrayBuffer(len > 0 ? (int) len : NodeFilter.SHOW_DOCUMENT_FRAGMENT);
    ...
    static {
        int i;
        UNRESERVED = new BitSet(NodeFilter.SHOW_DOCUMENT);
        PUNCT = new BitSet(NodeFilter.SHOW_DOCUMENT);
        USERINFO = new BitSet(NodeFilter.SHOW_DOCUMENT);
        PATHSAFE = new BitSet(NodeFilter.SHOW_DOCUMENT);
        URIC = new BitSet(NodeFilter.SHOW_DOCUMENT);
        RESERVED = new BitSet(NodeFilter.SHOW_DOCUMENT);
        URLENCODER = new BitSet(NodeFilter.SHOW_DOCUMENT);
        for (i = 97; i <= 122; i++) {
            UNRESERVED.set(i);
        }
        for (i = 65; i <= 90; i++) {
