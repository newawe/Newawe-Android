...
class IntHashMap {
    ...
    public boolean containsKey(int key) {
        Entry[] tab = this.table;
        int hash = key;
        for (Entry e = tab[(ASContentModel.AS_UNBOUNDED & hash) % tab.length]; e != null; e = e.next) {
            if (e.hash == hash) {
                return true;
            }
        }
        ...
        while (true) {
            ...
            if (i > 0) {
                ...
                while (old != null) {
                    Entry e = old;
                    old = old.next;
                    int index = (e.hash & ASContentModel.AS_UNBOUNDED) % newCapacity;
                    e.next = newMap[index];
                    newMap[index] = e;
                }
                i = i2;
    ...
    public Object put(int key, Object value) {
        Entry[] tab = this.table;
        int hash = key;
        int index = (hash & ASContentModel.AS_UNBOUNDED) % tab.length;
        ...
        if (this.count >= this.threshold) {
            rehash();
            tab = this.table;
            index = (hash & ASContentModel.AS_UNBOUNDED) % tab.length;
        }
    ...
    public Object remove(int key) {
        Entry[] tab = this.table;
        int hash = key;
        int index = (ASContentModel.AS_UNBOUNDED & hash) % tab.length;
        Entry prev = null;
        for (Entry e = tab[index]; e != null; e = e.next) {
            if (e.hash == hash) {
                if (prev != null) {
